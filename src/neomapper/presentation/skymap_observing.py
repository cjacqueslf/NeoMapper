...
from neomapper.application.ephemerides import query_horizons, query_horizons_range
from neomapper.domain.observing import AU_KM, observational_window_and_best, observing_night_window_utc
...
def build_sky_figure(object_query, utc_text, output_png=None, dpi=130, watermark_text="ASTRONEOS",
    ...
    best_time = None
    best_label = None
    best_reason = "not_calculated"
    window_start = None
    window_stop = None
    night_label = None
    try:
        night_start, night_stop, night_date = observing_night_window_utc(t, float(reference_lon))
        night_label = f"{night_date:%Y-%m-%d}/{(night_date + timedelta(days=1)):%Y-%m-%d}"
        eph_range = query_horizons_range(object_query, night_start, night_stop, step="5m")
        observation = observational_window_and_best(
            eph_range, float(reference_lat), float(reference_lon),
            min_altitude_deg=float(_unused.get("obs_min_altitude", 0.0)),
            sun_limit_deg=float(_unused.get("obs_sun_altitude_limit", -12.0)),
            height_m=float(reference_alt),
        )
        window_start = observation.get("window_start") or night_start
        window_stop = observation.get("window_stop") or night_stop
        best = observation.get("best")
        best_reason = getattr(best, "reason", "not_calculated")
        if best and best.altitude_deg is not None:
            best_altitude = float(best.altitude_deg)
            if best.time is not None:
                best_time, best_label = format_time_for_map(
                    best.time, time_mode, reference_lat, reference_lon
                )
    except Exception as exc:
        best_reason = f"error: {exc}"

    window_start_display = None
    window_stop_display = None
    if window_start is not None:
        window_start_display = format_time_for_map(
            window_start, time_mode, reference_lat, reference_lon
        )[0]
    if window_stop is not None:
        window_stop_display = format_time_for_map(
            window_stop, time_mode, reference_lat, reference_lon
        )[0]

    result={
        "targetname":eph.targetname,"utc":eph.utc_iso,"display_time":shown,
        "display_label":label,"geo_km":km,"vmag":eph.vmag,
        "ephemeris_source":eph.source,
        "current_altitude":obj_alt,"current_azimuth":obj_az,
        "current_ra_deg":float(eph.ra_deg),"current_dec_deg":float(eph.dec_deg),
        "sun_altitude":sun_alt,
        "solar_system":solar_system,"map_type":"sky",
        "best_altitude":best_altitude,
        "best_altitude_display_time":best_time,
        "best_altitude_display_label":best_label,
        "best_altitude_reason":best_reason,
        "observing_night_label":night_label,
        "observing_window_start_display":window_start_display,
        "observing_window_stop_display":window_stop_display,
        "obs_min_altitude":float(_unused.get("obs_min_altitude", 0.0)),
        "obs_sun_altitude_limit":float(_unused.get("obs_sun_altitude_limit", -12.0)),
        "suggested_filename":suggest_output_filename(object_query,utc_text,eph.targetname),
        "output":output_png,"pixel_size":pixel_size,
    }
    if output_png:
